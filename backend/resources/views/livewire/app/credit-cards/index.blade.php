                ...
                @forelse ($cards as $card)
                    <div class="grid grid-cols-12 items-center gap-2 px-4 py-3 text-sm">
                        <div class="col-span-4 min-w-0">
                            <div class="truncate font-medium text-slate-900">{{ $card->name }}</div>
                            <div class="text-xs text-slate-500">
                                @if ($card->limit_type !== 'sem_limite' && $card->limit_amount !== null)
                                    Limite: R$ {{ number_format((float) $card->limit_amount, 2, ',', '.') }} ({{ $card->limit_type }})
                                @else
                                    Sem limite
                                @endif
                            </div>
                        ...
                        <div class="col-span-2 text-slate-700">{{ $card->brand ?? '-' }}</div>
                        <div class="col-span-2 text-slate-700">{{ $card->last4 ? '**** '.$card->last4 : '-' }}</div>
                        <div class="col-span-1 text-slate-700">{{ $card->due_day ?? '-' }}</div>
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center">
            ...
            <div class="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <div class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Editar cartão' : 'Novo cartão' }}</div>
                ...
                <div class="mt-6 grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 focus:border-brand-600 focus:outline-none"
                        />
                        @error('last4') <div class="mt-1 text-xs text-red-600">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('limit_type') <div class="mt-1 text-xs text-red-600">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 focus:border-brand-600 focus:outline-none"
                        />
                        @error('limit_amount') <div class="mt-1 text-xs text-red-600">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 focus:border-brand-600 focus:outline-none"
                            @disabled($type === 'debito')
                        />
                        @error('due_day') <div class="mt-1 text-xs text-red-600">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        @error('monthly_limit') <div class="mt-1 text-xs text-red-600">{{ $message }}</div> @enderror
                    </div>
                </div>
