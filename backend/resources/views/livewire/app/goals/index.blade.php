                    ...
                    <div class="grid grid-cols-12 items-center gap-2 px-4 py-3 text-sm">
                        <div class="col-span-5 min-w-0">
                            <div class="flex items-center gap-2">
                                <a href="{{ url('/app/goals/'.$g->id) }}" class="truncate font-semibold text-slate-900 hover:text-brand-700">
                                    {{ $g->name }}
                                </a>
                                @if ($near)
                            ...
                            @if ($g->end_date)
                                <div class="mt-1 text-xs text-slate-500">Prazo: {{ optional($g->end_date)->format('d/m/Y') }}</div>
                            @endif
                        </div>
                        <div class="col-span-3">
                            <div class="flex items-center gap-3">
                                <div class="h-2 w-full rounded-full bg-slate-100">
                                    <div class="h-2 rounded-full bg-brand-600" @style(['width: '.$pctClamped.'%'])></div>
                                </div>
                                <div class="w-14 text-right text-xs font-semibold text-slate-700">{{ number_format($pct, 0) }}%</div>
                            </div>
                            <div class="mt-1 text-xs text-slate-500">
                                R$ {{ number_format($saved, 2, ',', '.') }} / R$ {{ number_format($target, 2, ',', '.') }}
                            </div>
                        </div>
                        ...
                        <div class="col-span-2 flex justify-end gap-2">
                            ...
                            <button
                                type="button"
                                wire:click="openWithdrawTx({{ $g->id }})"
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center">
            ...
            <div class="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <div class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Editar meta' : 'Nova meta' }}</div>
                ...
                <div class="mt-6 grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 focus:border-brand-600 focus:outline-none"
                        />
                        @error('start_date') <div class="mt-1 text-xs text-red-600">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 focus:border-brand-600 focus:outline-none"
                        />
                        @error('end_date') <div class="mt-1 text-xs text-red-600">{{ $message }}</div> @enderror
                    </div>
                </div>
    ...
    @if ($txModalOpen)
        <div class="fixed inset-0 z-[60] flex items-center justify-center">
            ...
            <div class="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <div class="text-lg font-semibold text-slate-900">{{ $tx_type === 'credito' ? 'Adicionar valor na meta' : 'Retirar valor da meta' }}</div>
                        <div class="mt-1 text-sm text-slate-500">{{ $txGoalName }}</div>
                    </div>
                    <button
                        type="button"
                        class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-900 hover:bg-slate-50"
                ...
                <div class="mt-6 grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        @error('tx_status') <div class="mt-1 text-xs text-red-600">{{ $message }}</div> @enderror
                    </div>
                </div>
